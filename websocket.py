...
@sync_to_async
def save_student_result(academic_exam, competitive_exam, student, score, result,time_duration):
...
async def save_results(room_id, student_id, score, user_info,time_duration):
    try:
        if user_info.batch_id:
            existing_id = await get_competitive_room(room_id)
        ...
        if (competitive_exam or academic_exam) and student_instance:
            await save_student_result(academic_exam, competitive_exam, student_instance, score, result,time_duration)
        ...
        for result in results:
            ...
            if student:
                result_data = {
                    ...
                    "time_duration": int(result.time_duration * 1000) if result.time_duration is not None else None  # Convert time_duration to milliseconds
                }
                results_data.append(result_data)

        # Sorting by score in descending order and time_duration in ascending order within the same score
        sorted_results_data = sorted(results_data, key=lambda x: (x['score'], -x['time_duration'] if x['time_duration'] is not None else float('inf')), reverse=True)

        rank = 1
        prev_score = prev_time_duration = None

        for result in sorted_results_data:
            if result['score'] != prev_score:
                result['rank'] = rank
            elif result['time_duration'] != prev_time_duration:
                result['rank'] = rank
            prev_score = result['score']
            prev_time_duration = result['time_duration']
            rank += 1
            ...
            if message.startswith("start"):
                ...
                try:
                    ...
                    if len(parts) >= 3:
                        ...
                        current_time_duration = float(parts[3])
                        time_duration += current_time_duration
